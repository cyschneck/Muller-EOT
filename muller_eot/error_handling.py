...
def errorHandlingEOT(eccentricity=None,
	...
	if eccentricity is not None and type(eccentricity) != int and type(eccentricity) != float:
		logger.critical(f"\nCRITICAL ERROR, [eccentricity]: Must be a int or float, current type = '{type(eccentricity)}'"))
		exit()
	...
	if obliquity is not None and type(obliquity) != int and type(obliquity) != float:
		logger.critical(f"\nCRITICAL ERROR, [obliquity]: Must be a int or float, current type = '{type(obliquity)}'")
		exit()
	...
	if orbit_period is not None and type(orbit_period) != int and type(orbit_period) != float:
		logger.critical(f"\nCRITICAL ERROR, [orbit_period]: Must be a int or float, current type = '{type(orbit_period)}'")
		exit()
...
def errorHandlingPlotEOT(eot_dict=None,
	...
	if plot_title is not None and type(plot_title) != str:
		logger.critical(f"\nCRITICAL ERROR, [plot_title]: Must be a str, current type = '{type(plot_title)}'")
		exit()

	# Ensure that plot_x_title is a string
	if plot_x_title is not None and type(plot_x_title) != str:
		logger.critical(f"\nCRITICAL ERROR, [plot_x_title]: Must be a str, current type = '{type(plot_x_title)}'")
		exit()

	# Ensure that plot_y_title is a string
	if plot_y_title is not None and type(plot_y_title) != str:
		logger.critical(f"\nCRITICAL ERROR, [plot_y_title]: Must be a str, current type = '{type(plot_y_title)}'")
		exit()

	# Ensure that all show_plot is a boolean ["True", "False"]
	if type(show_plot) != bool:
		logger.critical(f"\nCRITICAL ERROR, [show_plot]: Must be a bool, current type = '{type(show_plot)}'")
		exit()
	...
	if type(fig_plot_color) != str:
		logger.critical(f"\nCRITICAL ERROR, [fig_plot_color]: Must be a string, current type = '{type(fig_plot_color)}'")
		exit()

	# Ensure that all figsize_n is a float or int
	if type(figsize_n) != int and type(figsize_n) != float:
		logger.critical(f"\nCRITICAL ERROR, [figsize_n]: Must be a int or float, current type = '{type(figsize_n)}'")
		exit()

	# Ensure that all figsize_dpi is a float or int
	if type(figsize_dpi) != int and type(figsize_dpi) != float:
		logger.critical(f"\nCRITICAL ERROR, [figsize_dpi]: Must be a int or float, current type = '{type(figsize_dpi)}'")
		exit()

	# Ensure that the effect title type is a string
	if save_plot_name!= None and type(save_plot_name) != str:
		logger.critical(f"\nCRITICAL ERROR, [save_plot_name]: Must be a str, current type = '{type(save_plot_name)}'")
		exit()
