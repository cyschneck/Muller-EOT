...
if __name__ == '__main__':
	...
	for planet in planet_lst:
		if True:# planet[0] == "Earth":
			...
			eccentricity_y = muller_eot.calculateDifferenceEOTMinutes(planet[1], 0, orbital_period_planet)
			muller_eot.plotEOT(planet[0],
								orbital_period_planet,
								eccentricity_y, 
								"Eccentricity ({0})".format(planet[1]),
								"examples/{0}_eccentricity_testing".format(planet[0].lower()))

			# Effect of Obliquity
			obliquity_y = muller_eot.calculateDifferenceEOTMinutes(0, planet[2], orbital_period_planet)
			muller_eot.plotEOT(planet[0],
								orbital_period_planet,
								obliquity_y,
								"Obliquity ({0}°)".format(planet[2]),
								"examples/{0}_obliquity_testing".format(planet[0].lower()))
			...
			muller_eot.plotEOT(planet[0],
								orbital_period_planet,
								eot_combined_y,
								"Eccentricity ({0}) and Obliquity ({1}°)".format(planet[1], planet[2]),
								"examples/{0}_eot_testing".format(planet[0].lower()))
